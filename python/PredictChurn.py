...
def main():
    ...
    # We pull from the view we created, not the raw data!
    ...
    # SAVE the IDs before we drop them so we know who is who!

    customer_ids = df['customer_id']

    # Drop IDs and prevent Data Leakage by dropping the pre-calculated 'churn_risk'
    ...
    # Machine Learning models can't read text like "Male" or "North". 
    # pd.get_dummies converts these text columns into 1s and 0s automatically.
    ...
    # --- NEW CODE GOES HERE, STILL INDENTED INSIDE main() ---
    print("\n" + "="*40)
    ...
    # Create a DataFrame to make it look nice, and sort it
    feature_df = pd.DataFrame({
        'Feature': feature_names,
        'Importance': importances
    }).sort_values(by='Importance', ascending=False)
    
    # Print the top 10 reasons customers are leaving
    ...
    # Predict the exact percentage risk (0% to 100%) for EVERY customer
    probabilities = model.predict_proba(X_encoded)[:, 1] 
    
    # Create a new clean table with just the Customer ID and their Risk Score
    predictions_df = pd.DataFrame({
        'customer_id': customer_ids,
        'Churn_Probability': probabilities
    })
    
    # Save this table directly into your SQL Server database!
    ...
    print("Success! Your ML predictions are now waiting in SQL Server for Tableau.")
    ...
    # 3. Export the customer details so we know who to email
